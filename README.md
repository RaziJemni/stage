...
- explicit Airbnb, Booking.com, Vrbo, and Expedia read-only iCalendar feed sources, using the existing asynchronous import and truthful sync-health workflow;
...
Property management is connected to the authenticated API: managers can create, edit, archive, and restore persistent company properties, while staff can view company properties. Properties support multi-owner attribution, linking property entities to company-scoped `Owner` contact records (`commission_percentage`, contact info, notes) without portal credentials, with owner attribution displayed on property cards, detail dossiers, and edit controls. The property detail view fetches live upcoming bookings for the property directly from the authenticated calendar API with truthful loading, empty, and retryable failure states. Staff and managers can also create, update, and cancel persistent manual/direct bookings for active company properties; direct bookings record operational folio tracking including payment status (`unpaid`, `deposit_received`, `paid_in_full`), total and paid amounts with 3-decimal currency precision (e.g. TND), payment method, and calculated outstanding balance, with blocked periods strictly isolated from payment tracking. Authenticated staff can generate and download or print branded direct booking confirmation vouchers, receipts, and invoices (`text/html` with `@media print` A4 styling, unique reference code `VAY-YYYYMM-XXXX`, itemized stay breakdown in TND, payment status, amount paid, balance due, and cancelled watermarks) directly from the Calendar reservation drawer and Property Detail upcoming bookings list. The backend availability service uses active reservation and blocked-period data, and an authenticated availability check endpoint validates date ranges against existing stays in the property timezone. The portfolio Calendar screen is connected to persisted properties, bookings, blocked periods, feed health, and conflicts, with responsive filtering, manual booking controls, upfront conflict detection that warns operators of overlapping stays and requires explicit confirmation before recording conflicts, and payment status badges with folio summaries and unpaid balance callouts in the reservation drawer. Managers can configure supported iCalendar feeds through the authenticated API; Celery imports, updates, and cancels events asynchronously while recording truthful synchronization health. The backend detects, lists, and acknowledges persistent pairwise booking conflicts, while automatically resolving conflicts when their overlap disappears. Manager Settings Integrations now reads persisted calendar-feed health, supports URL-private iCalendar setup and refresh requests, and reports WhatsApp simulator, test, or explicitly unconfigured production mode. Authenticated staff can view company-scoped conversation lists and history, filter unread and handling states, mark shared unread messages as read, manually take over a conversation, and save replies. A signed local WhatsApp simulator persists deterministic inbound messages but does not connect to WhatsApp or send outbound messages. Safe simulator messages are processed asynchronously into grounded French, English, Arabic, Italian, or German replies from authorized property facts or the backend availability service; replies are recorded as queued, not delivered. Missing facts, sensitive requests, and provider failures are switched to manual handling with an escalation reason. Production model configuration is isolated behind an adapter and is not an approved WhatsApp connection. Maintenance creates and lists persistent company tickets with deductible resolution cost tracking (`cost` with non-negative check constraint), holds chatbot suggestions for staff confirmation or rejection, allows staff to send reviewed template updates to guests for linked tickets, and allows staff to manage contractor contacts and assign or reassign active contacts while preserving history. Staff can filter the connected maintenance board, transition tickets through the approved lifecycle, and view immutable status history; terminal transitions end active contractor assignments. Contractors remain external contacts without Vayca login.

Managers can configure company-scoped direct-booking base rates, Friday/Saturday adjustments, minimum stays, and seasonal overrides. Calendar operators request an itemized server-side recommendation before saving a direct reservation; a saved profile requires an explicit approval, and any manual total override requires a reason stored with the immutable booking pricing decision. This applies only to direct bookings: Airbnb, Booking.com, Vrbo, and Expedia iCalendar feeds remain read-only and do not receive rate updates.

The Dashboard displays company-scoped attention items, today’s arrivals and departures, active properties, and a derived rolling 30-day portfolio occupancy rate tile with calm operational styling. A dedicated Owner Statements view aggregates monthly gross booking revenue for completed stays, agency management commissions, and deductible maintenance expenses into truthful per-owner payout summaries, with itemized statement inspection (property, booking, and ticket breakdowns) and direct CSV export downloads. Managers can generate instant 1-click shareable portal links, manually dispatch statement emails, or print branded A4 statements. Celery Beat automatically runs scheduled monthly statement dispatches on the 1st of each month, sending bilingual (FR/EN) performance reports with itemized KPIs and secure portal links to active partner owners. External property owners access their statements via a dedicated, token-secured, read-only Owner Statement Portal (`/owner/statements?token=...`), which validates time-limited SHA-256 token digests and offers responsive stay/maintenance tabs, CSV exports, and printable certified statements without requiring system login credentials or user accounts. Manager Settings includes an Owners tab for managing partner contacts and commission rates. Property-level filtering allows operators to inspect operations, attention items, arrivals, departures, and occupancy metrics for individual properties or the entire portfolio. Supervision analytics compute commercial occupancy percentages (`occupied_room_nights / available_room_nights * 100`) while excluding maintenance blocked periods and archived properties, adhering to the company timezone and handling edge cases cleanly. Post-stay review-request sequences automatically evaluate completed guest reservations, dispatch safety-guarded multilingual review requests across messaging channels with idempotency, and suppress requests when unresolved urgent maintenance or complaints occurred during the stay. The workstation interface standardizes sticky 64px `WorkstationHeader` breadcrumbs and actions across all primary views (Dashboard, Calendar, Messages, Conversation Thread, Maintenance, Properties, Property Detail, and Settings), with complete French (`fr`) default localization tailored for Tunisian property operators, symmetrical dynamic English (`en`) switching, interactive language selection in Settings, and persistent user language preferences stored in PostgreSQL across user sessions and devices.
...
Outbound guest messaging and webhook ingestion are powered by a multi-provider WhatsApp adapter supporting local simulation, Meta WhatsApp Business Cloud API, and Twilio WhatsApp Messaging. Inbound webhooks resolve property context through active guest stays or configured fallbacks, while staff and grounded chatbot replies transmit outbound messages to guest WhatsApp numbers, transitioning delivery states to `SENT` or `FAILED` and tracking provider delivery callbacks (`delivered`, `read`, `failed`) idempotently. For interactive defense demonstrations and offline evaluation, an unauthenticated mobile guest simulator (`/simulator/whatsapp`) delivers an authentic WhatsApp Web client interface with green header branding (`#075E54`), dynamic villa selection, configurable guest phone numbers, pre-filled inquiry chips (WiFi, pool, check-out, AC failure), and real-time conversation synchronization. Evaluators can scan a presentation QR code directly from the workstation header modal (`SimulatorQrModal`) using any smartphone camera to open the interface, chat live with the grounded AI assistant, or observe real-time staff takeover and ticket escalation in the workstation Inbox.
