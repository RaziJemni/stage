  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FAF8F5]">
      ...
      <div className="flex-1 p-6 max-w-5xl w-full mx-auto space-y-5">
        ...
        {!loading && !error && <section className="space-y-3 rounded-2xl border border-[#EBE6DD] bg-white p-5 shadow-sm">{messages.length === 0 ? <p className="py-8 text-center text-sm text-[#78716C]">No messages have been recorded yet.</p> : messages.map((message) => {
            ...
            <article key={message.id} className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${message.direction === 'outbound' ? 'ml-auto bg-[#0F3D5E] text-white' : 'bg-[#FAF8F5] text-[#1C1B18]'}`}>
              {isReviewRequest && (
                <div className="mb-1.5 inline-flex items-center gap-1 rounded-full bg-amber-400/20 px-2 py-0.5 text-[10px] font-bold text-amber-200">
                  <Star className="h-3 w-3 fill-amber-300 text-amber-300" /> Post-stay review request
                </div>
              )}
              <p>{message.content}</p>
              <p className={`mt-2 text-[11px] ${message.direction === 'outbound' ? 'text-[#DCEDF5]' : 'text-[#78716C]'}`}>
                {isReviewRequest ? 'Automated Sequence' : message.sender_type} · {formatTime(message.created_at)} · {message.delivery_status}
              </p>
            </article>
          );
        })}</section>}
